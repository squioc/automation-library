...
def trigger(symphony_storage):
    ...
    trigger.configuration = {"frequency": 604800, "chunk_size": 20}
    trigger.log = Mock()
...
def test_fetch_log(trigger):
    ...
    call_args = trigger.send_event.call_args.kwargs
    assert call_args["event"] == {"logs_path": "event.json"}
    assert call_args["event_name"] == "New Imperva events"
...
def test_fetch_log_should_chunk_events(trigger):
    trigger.logs_file_index.indexed_logs = Mock(return_value=["42_42.log"])
    trigger.file_downloader.request_file_content = Mock(
        return_value=b"lorem:ipsum|==|\n" + zlib.compress(b"\n".join([b"a log"] * 40))
    )
