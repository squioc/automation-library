...
class LogsDownloader(Trigger):
    ...
    def _send_logs(self, logs: list, event_name: str, to_file: bool | None = True):
        if not to_file:
            self.send_event(event={"logs": logs}, event_name=event_name)
            return

        # save event in file
        work_dir = self.data_path.joinpath("imperva_events").joinpath(str(uuid.uuid4()))
        work_dir.mkdir(parents=True, exist_ok=True)

        event_path = work_dir.joinpath("event.json")
        with event_path.open("w") as fp:
            fp.write(orjson.dumps(logs).decode("utf-8"))
    ...
    def handle_log_decrypted_content(self, decrypted_file):
        ...
        for logs in self._chunk_events(events_list, self.chunk_size):
            self._send_logs(logs=list(logs), event_name="New Imperva events")
        ...
        if file_encryption_key == -1:
            ...
            try:
                ...
                content_decrypted_sym_key = rsa_private_key.decrypt(
                    base64.b64decode(bytes(content_encrypted_sym_key, "utf-8")), padding.PKCS1v15()
                )
